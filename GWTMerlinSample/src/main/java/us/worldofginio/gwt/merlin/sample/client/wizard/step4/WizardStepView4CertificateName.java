...
public class WizardStepView4CertificateName extends
		AbstractWizardStepView<SampleModel> {

	@UiField
	@Path("certificateName")
	TextBox txtCertificateName;

	interface Binder extends UiBinder<Widget, WizardStepView4CertificateName> {
		// GWT UI binder
	}

	private static final Binder UIBINDER = GWT.create(Binder.class);

	interface AliasSectionContentEditorDriver extends
			SimpleBeanEditorDriver<SampleModel, WizardStepView4CertificateName> {
		// GWT data binding driver
	}

	// don't make this variable static as this leads to strange JS NPEs
	private final AliasSectionContentEditorDriver driver = GWT
			.create(AliasSectionContentEditorDriver.class);

	interface Presenter extends WizardStepPresenter {
	}

	public WizardStepView4CertificateName() {
		super();
